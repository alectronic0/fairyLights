...
public class LightArray {
    ...
    public LightArray(int l) {
        ...
        for (int i = 0; i < l; i++) {lights.add(new Light(getColour(i)));}
    }
...
//GETTERS
    public String getColour(int i) {return listOfColours[i % listOfColours.length];}


    public int getLightArraySize(){return lights.size();}
    ...
    public ArrayList<Integer> getColoursPos(String colour){
        ...
        if(colourInList(colour)){
            for(int i = 0; i < lights.size(); i++){
                ...
                if(li.isThisColour(colour)){colourPos.add(i);}
            }
        }
        return colourPos;
    }
    ...
    public boolean colourInList(String colour){
        for (String col : listOfColours){
            if(colour.toLowerCase().equals(col.toLowerCase())){return true;}
        }
        return false;
    }
